...
class GAN:
        ...
        for _ in range(1):
            with tf.GradientTape() as d_tape:
                ...
                fake_output = tf.concat(
                    [generated_data_reshape, tf.cast(yc, tf.float32)], axis=1
                )
                real_y_reshape = tf.reshape(
                    real_price, [real_price.shape[0], real_price.shape[1], 1]
                )
                real_output = tf.concat(
                    [tf.cast(real_y_reshape, tf.float32), tf.cast(yc, tf.float32)],
                    axis=1,
                )
    ...
    def train(self, X_train, y_train, yc, epochs):
        ...
        return (
            Predicted_price,
            Real_price,
            np.sqrt(mean_squared_error(Real_price, Predicted_price))
            / np.mean(Real_price),
        )
