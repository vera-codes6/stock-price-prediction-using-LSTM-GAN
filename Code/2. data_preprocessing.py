...
dataset.isnull().sum()
dataset.iloc[:, 1:] = (
    pd.concat([dataset.iloc[:, 1:].ffill(), dataset.iloc[:, 1:].bfill()])
    .groupby(level=0)
    .mean()
)
...
def predict_index(dataset, X_train, n_steps_in, n_steps_out):
    ...
    train_predict_index = dataset.iloc[
        n_steps_in : X_train.shape[0] + n_steps_in + n_steps_out - 1, :
    ].index
...
(
    index_train,
    index_test,
) = predict_index(dataset, X_train, n_steps_in, n_steps_out)
# %% --------------------------------------- Save dataset -----------------------------------------------------------------
